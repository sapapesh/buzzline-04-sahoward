...
"{\"message\": \"I just shared a meme! It was amazing.\", \"author\": \"Charlie\", \"timestamp\": \"2025-01-29 14:35:20\", \"category\": \"humor\", \"sentiment\": \"0.87\", \"keyword_mentioned\": \"meme\", \"message_length\": \"42\"}"
...
# Store sentiment values grouped by category
...
def update_chart():
    ...
    # Get the categories and average sentiment values
    categories = list(category_sentiments.keys())
    avg_sentiments = [
        sum(vals) / len(vals) if vals else 0
        for vals in category_sentiments.values()
    ]

    # Create a bar chart using the bar() method.
    # Pass in the x list, the y list, and the color
    ax.bar(categories, avg_sentiments, color="red", edgecolor="black")

    # Use the built-in axes methods to set the labels and title
    ax.set_xlabel("Category")
    ax.set_ylabel("Average Sentiment")
    ax.set_title("Real-Time Average Sentiment Per Category by Sarah Howard")
    ax.set_ylim(0, 1)  # since sentiment is in [0,1]
...
def process_message(message: str) -> None:
    ...
    try:
        ...
        if isinstance(message_dict, dict):
            ...
            sentiment = float(message_dict.get("sentiment", 0.0))

            logger.info(f"Message category: {category}, sentiment: {sentiment}")

            # Append sentiment to category list
